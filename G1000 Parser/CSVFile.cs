...
namespace Glass_Cockpit {
	public class CSVFile { // TODO: do parsing in one pass rather than looping through Entries for each property
		...
		private async Task loadData() {
			try {
				...
				for (int i = 3; i < lines.Length; i++) {
					...
					if (temp.Length == Entry.ENTRY_LENGTH)
						this.data.Add(new Entry(temp));
				}
